...
```
It is a good value, only having obtained a lower value with the non-Bayesian step model.

## Using a manual prior distibution

Using a manual prior distribution **igual tenemos que buscar una prior mejor**
...
reg.prior <- list(
    ...
    V = diag(1 , length(x_train) + 1)))

data <- as.data.frame(sc)
scaled_tets <- scale(test)
base_bayes_model_mprior <- MCMCglmm(f, prior = reg.prior,
                                           data = data, verbose = FALSE)
bayesian_mprior_preds   <- predict(base_bayes_model_mprior, scale(test))
...
rmse_bayesian <- data.frame(
    ...
    uninformative = rmse(bayesian_mprior_preds, y_test),

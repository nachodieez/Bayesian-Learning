...
temp_fit <- data_ts %>% 
    model(arima_310 = ARIMA(temp ~ 1 + pdq(3, 1, 0) + PDQ(0, 0, 0) + co2 + lag(co2, 12))) 
# Forecast 5 years
...
temp <- read.csv("https://data.giss.nasa.gov/gistemp/tabledata_v4/GLB.Ts+dSST.csv",
                 header = TRUE, skip = 1, sep = ",", na.strings = "***")
temp_ts <- ts(as.vector(t(temp[,2:13])), start = c(1958, 3), frequency = 12) %>% 
    as_tsibble() %>% drop_na()
temp    <- temp_ts %>% as.ts()
```
...
# Plot data
#
ts.plot(temp,lwd=2,ylab='', xlab = "Time")
#
...
# 
pred1 <- predict(model1, horizon = 60)
...
Now, we will try the Bayesian version of ARIMA. We will use a training and a test partition, and we will compare it to a naive model.
